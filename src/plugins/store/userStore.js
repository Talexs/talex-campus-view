...
export default () => {
  ...
  function process() {
    if (tStore.theme.adapt === ThemeAdapt.NONE) changeTheme(tStore.theme);
  ...
  if (tStore.loggedIn)
    !asyncInit &&
      window.$tipper.tip("正在同步个人设置...", {
        loading: async (func) => {
          if (!(tStore.theme instanceof Object))
            tStore.theme = { name: "light", adapt: ThemeAdapt.NONE };
